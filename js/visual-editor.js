...
(function () {
  ...
  function bindText(el) {
    ...
    el.addEventListener('blur', () => {
      ...
      } else if (el.dataset.cmsPage && window.TFC_PAGE_EDITOR) {
        window.TFC_PAGE_EDITOR.onFieldChange(path, el.dataset.cmsType || 'text', val);
      } else if (path) {
      ...
      openMediaPanel(el, {
        ...
        setValue: (url) => {
          if (filmSlug && field) {
            ...
            window.TFC_PAGE_EDITOR.onFieldChange(path, isVideo ? 'video' : 'image', url);
          } else if (path) {
  ...
  async function publish() {
    ...
    try {
      ...
      if (window.TFC_PAGE_EDITOR) saves.push(window.TFC_PAGE_EDITOR.saveAll());
  ...
  async function startEditor() {
    ...
    // Re-bind when page-editor finishes auto-tagging elements
    document.addEventListener('tfc:page-tagged', () => bindAll());
